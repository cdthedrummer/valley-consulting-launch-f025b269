...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${req.headers.get("origin")}/ai/dashboard?reactivated=true`,
      cancel_url: `${req.headers.get("origin")}/ai/dashboard?reactivated=false`,
    });

    logStep("Created reactivation checkout session", { sessionId: session.id, url: session.url });
